...
First, we see that all references have now been replaced with `AtomicPtr`, which is essentially a wrapper around a raw pointer, which is updated and read atomically (furthermore, this is not `std::sync::atomic::AtomicPtr`, but rather `haphazard::AtomicPtr` which wraps the one in std to also integrate with hazard pointers nicely). As a minor optimization, `next` is just an `AtomicPtr<Node<T>>` instead of an `Option<AtomicPtr<Node<T>>>` as we can essentially use `std::ptr::null` instead of `None`.

The most interesting change from the pseudocode, in my opinion, is that `data` now has the type `ManuallyDrop<T>` instead of just `T`. Why is this? Well, think about the ownership of some data and its enqueued node. When the node is created, it gets ownership of the data. When the node is dequeued, we would like to own it, and take over the ownership of its `data` field, not requiring some strange `ManuallyDrop<T>`. However, we don't get this owned version of the node when it is dequeued. Instead the node stays as `queue.head` for a bit after being dequeued, and even after being unlinked, its reclamation is deferred and handled by the hazard pointers. Therefore, the data will be returned from `dequeue`, but also live on for a while in the node until the node is freed. However, the logical ownership of the data after being returned by a dequeue is moved out of the queue, so to avoid double-free, we wrap the node data in `ManuallyDrop`, making sure it is not freed together with the node (leading to undefined behavior).
...
impl<T> MSQueue<T> {
    pub fn new() -> Self {
        let dummy = Box::new(Node::new(unsafe { std::mem::zeroed() })).into_raw();
        Self {
...
For the node, we simply wrap `data` in a new `ManuallyDrop`, and initialize an `AtomicPtr` as a null pointer. In the queue, similarly to the pseudocode, we create a dummy node that both `head` and `tail` point to. This dummy node is required for the algorithm (if we had let them both be `null`, we would need to update them both when enqueueing an item, rather than just `dummy.next`), and as its `data` is never read, it can be initialized to whatever (instead of `ManuallyDrop`, we could also have used `MaybeInit` in much the same way). Note also that we use `Box::into_raw`, which returns the raw pointer of a `Box`, and is very useful for us to get around ownership in the linked list.
...
Another interesting part is how we read out the `data`. There felt as if there were many ways to try this, but using `std::ptr::read` on the raw pointer was a simple and nice solution. This takes the ownership of `T` from `&T`, which is scary as we now have two owned values of `data`. However, due to using `ManuallyDrop`, and algorithmic guarantees, we ensure that our code will never read (or free) the `data` in this node again, making the ownership transfer safe.

Furthermore, we here see that we use `retire` to free the unlinked node from the queue. This free is then deferred until no hazard pointer has a reference to it. It requires `unsafe` as it can have extremely bad consequences if the node is unlinked before it completely unreachable from _all_ parts of the data structure.
...
This helped find one bug with CAS in the development of the code above. But if you use all the code in its current state, and add some simple test, you will still get an error: A memory leak! In hindsight, this should be obvious, as we never drop the nodes in a queue when it is dropped, so let's fix that!
