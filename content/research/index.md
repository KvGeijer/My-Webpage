+++
title = "My Research"
description = "A few highlights from my time in academia."
template = "standalone-page.html"
+++

# Awards
- Nominated for the 2026 Pedagogical Prize by the Chalmers IT Student Section.
- Winner of the Parallel SSSP track at the [FastCode Programming Challenge](https://fastcode.org/events/fastcode-challenge/) at [PPoPP 2025](https://ppopp25.sigplan.org/).
- Best Paper Award at [Euro-Par 2024](https://2024.euro-par.org/).

# Publications

- [**Semantic Relaxation of Concurrent Data Structures: Efficient and Elastic Designs**](../licentiate/semantic-relaxation-of-concurrent-data-structures:efficient-and-elastic-designs.pdf) (Licentiate thesis 2025). Kåre von Geijer.
- [**Elastic Relaxation of Concurrent Data Structures**](https://doi.org/10.1109/TPDS.2025.3587888) (TPDS 2025). Kåre von Geijer, Philippas Tsigas.
  - Links: [Open access paper](https://doi.org/10.1109/TPDS.2025.3587888), [published artifact](https://doi.org/10.5281/zenodo.15872817).
- [**Relax and don't Stop: Graph-Aware Asynchronous SSSP**](https://doi.org/10.1145/3711708.3723446) (FCPC at PPoPP 2025). Marco D'Antonio, Kåre von Geijer, Thai Son Mai, Philippas Tsigas, Hans Vandierendonck.
  - Won the competition for fastest parallel SSSP solver.
- [**Balanced Allocations over Efficient Queues: A Fast Relaxed FIFO Queue**](https://doi.org/10.1145/3710848.3710892) (PPoPP 2025). Kåre von Geijer, Philippas Tsigas, Elias Johansson, Sebastian Hermansson.
  - Links: [Open access paper](https://doi.org/10.1145/3710848.3710892), [accepted manuscript](./balanced-allocations-over-efficient-queues.pdf), [published artifact](https://doi.org/10.5281/zenodo.14223245).
- [**How to Relax Instantly: Elastic Relaxation of Concurrent Data Structures**](https://doi.org/10.1007/978-3-031-69583-4_9) (Euro-Par 2024). Kåre von Geijer, Philippas Tsigas.
  - Won the Euro-Par 2024 _Best Paper_ award.
  - Links: [Published paper](https://doi.org/10.1007/978-3-031-69583-4_9), [accepted manuscript](./how-to-relax-instantly.pdf), [extended pre-print](https://arxiv.org/abs/2403.13644), [published artifact](https://zenodo.org/doi/10.5281/zenodo.11547062).

# Master Thesis Supervision

I have had the pleasure to supervise a few students through their master's theses. These projects have often led to some interesting results, and have been very enjoyable to work with. Here is the list of projects I've supervised (links included for the ones available online):

- Scalable Parallel Branch-and-Bound with Relaxed Queues (2026). Ludvig Sandh.
- Understanding Work-Efficiency of label-correcting Single Source Shortest Path Algorithms (2026). Alfred Berglöf, Johan Berg.
- The Benefits and Limitations of Rust for Concurrent Data Structures (2026). Elliot Hultgren, Max Vallin Ek.
- The Psychology of Fear in Lock-Free Rust Programming (2026). Emil Björnlinger.
- [A Faster Breadth-First Search on Sparse Graphs](https://odr.chalmers.se/items/07e25284-38a8-47e2-a6ba-8f70b66eaba9) (2025). Simon Holst, Johan Selin.
- [Efficiently Calculating Relaxation Errors](https://odr.chalmers.se/items/65b16051-4f41-4cb0-b179-14f9ac3602d6) (2025). Elis Kleen, Victor Olin.
- [Accurate Linearizations for Relaxed FIFO queues](https://odr.chalmers.se/items/e15159b6-a63a-45fc-9e3a-5bc81d608e0f) (2025). Ida Dahl, Hanna Schaff.
- [Lock-Free Queues in Rust](https://odr.chalmers.se/items/b545180e-e966-44a6-8c18-1b6a16fcecbf) (2025) ([GitHub repo](https://github.com/dcs-chalmers/rusty-benchmarking-framework)). Gustavn Seffel, Willaim Berg.
- [Progress-Based Distributed Queues](https://odr.chalmers.se/items/08baff48-42da-4fa5-a3af-62340beed66a) (2024). Sebastian Hermansson, Elias Johansson.
- [Lock-Free Concurrency in Rust](https://odr.chalmers.se/items/df3197f6-d6fc-4242-9fc1-a93efc26b5c3) (2024). Lilly Jinstrand, Marcus Julin.
- Granular Contention Avoiding Priority Queues in Rust (2024). Sebastian Nielsen, Rasmus Rosengren.
- [Relaxed Priority Queue & Evaluation of Locks](https://odr.chalmers.se/items/131af978-319c-4032-a7de-09fbda1511b1) (2023). Andreas Rudén, Ludvig Andersson.